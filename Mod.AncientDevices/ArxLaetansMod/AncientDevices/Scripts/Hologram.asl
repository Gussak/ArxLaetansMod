...
ON INVENTORYUSE { Set £ScriptDebugLog "On_InventoryUse"
	...
	if (^inPlayerInventory == 1) {
		if(and(£AncientDeviceMode == "Hologram" && §bHologramInitialized == 0)) Set £AncientDeviceMode "_BecomeSignalRepeater_"
		GoSub FUNCmorph
		//PLAY "POWER_DOWN"
		//showlocals
...
ON COMBINE { Set £ScriptDebugLog "On_Combine:~^$PARAM1~"
	...
	//RANDOM §CreateChance {
		//if ( £AncientDeviceMode == "Hologram" ) { Set £AncientDeviceMode "Grenade"
			...
			//Set £FUNCnameUpdate_NameBase "HoloGrenade"
		...
		//} else { if ( £AncientDeviceMode == "Grenade" ) { Set £AncientDeviceMode "LandMine"
			...
			//Set £FUNCnameUpdate_NameBase "LandMine"
			//Set £Icon "HoloLandMine"
			//Set §AncientDeviceTriggerStep 1
			//PLAYERSTACKSIZE 9
		//} else { if ( £AncientDeviceMode == "LandMine" ) { Set £AncientDeviceMode "Teleport"
			//TWEAK SKIN "Hologram.tiny.index4000.boxLandMine" "Hologram.tiny.index4000.boxTeleport" 
			//Set £FUNCnameUpdate_NameBase "Teleport"
			//Set £Icon "HoloTeleport"
			//Set §AncientDeviceTriggerStep 1
			//PLAYERSTACKSIZE 6
		//} else { if ( £AncientDeviceMode == "Teleport" ) { Set £AncientDeviceMode "MindControl"
			...
			//Set £FUNCnameUpdate_NameBase "MindControl"
...
>>FUNCteleportToAndKillNPC { Set £ScriptDebugLog "~£ScriptDebugLog~;FUNCteleportToAndKillNPC" 
	...
	if(and(£HoverEnt != "none" && §HoverLife > 0)) {
		timerTeleportKillNPC -m 1 1333 DoDamage -l ~£HoverEnt~ 99999
		timerTeleportSelf    -m 1 2666 teleport    ~£HoverEnt~
		timerTeleportPlayer  -m 1 3999 teleport -p ~£HoverEnt~
...
>>FUNCmorph { Set £ScriptDebugLog "~£ScriptDebugLog~;FUNCmorph" 
	Set §AncientDeviceTriggerStep 0
	GoSub FUNCskillCheckAncientTech
	Set §CreateChance §FUNCskillCheckAncientTech_chanceSuccess_OUTPUT
	if (and(§Quality >= 4 && §ItemConditionSure == 5)) Set §CreateChance 100
