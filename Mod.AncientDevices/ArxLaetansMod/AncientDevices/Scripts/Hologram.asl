...
ON INVENTORYUSE { Set £ScriptDebugLog "On_InventoryUse"
	...
	if ( £AncientDeviceMode == "LandMine" ) {
		if ( §AncientDeviceTriggerStep == 1 ) {
			...
			§AncientDeviceTriggerStep == 1 //stop seek
		} }
		GoSub FUNCnameUpdate
		ACCEPT
	} else {
...
ON COMBINE { Set £ScriptDebugLog "On_Combine:~^$param1~"
	...
	RANDOM §CreateChance {
		if ( £AncientDeviceMode == "Hologram" ) { Set £AncientDeviceMode "Grenade"
			...
			Set §AncientDeviceTriggerStep 1
		} else { if ( £AncientDeviceMode == "Teleport" ) { Set £AncientDeviceMode "MindControlBats"
		...
		if(§AncientDeviceTriggerStep == 1) {
			if ( §Quality >= 4 ) {
				Set £FUNCnameUpdate_NameBase "~£FUNCnameUpdate_NameBase~+"
				Set £Icon "~£Icon~MK2"
			}
...
>>FUNCteleportToAndKillNPC {
	Set £HoverEnt "~^hover~"
	if(£HoverEnt != "none") {
		timerTeleportKillNPC -m 333 1 DoDamage -l "~£HoverEnt~" 99999
		timerTeleportSelf    -m 666 1 teleport     ~£HoverEnt~
		timerTeleportPlayer  -m 999 1 teleport -p  ~£HoverEnt~
	}
	RETURN
}
