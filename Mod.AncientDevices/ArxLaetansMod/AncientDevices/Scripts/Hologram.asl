...
>>TFUNCnameUpdate { GoSub FUNCnameUpdate ACCEPT } >>FUNCnameUpdate {
	...
	//if ( §bHologramInitialized == 1 ) {
		...
		//if(@AncientTechSkill >= 50) { //detailed info for nerds ;) 
			...
			if(§Identified == 1) {
				Set £ItemConditionDesc "~£ItemConditionDesc~(~§ItemConditionPercent~% ~§UseCount~/~§UseMax~ Remaining ~§UseRemain~) RT:~^gamedays~day(s) ~%02d,§hours~:~%02d,§minutes~:~%02d,§seconds~ raw:~^arxtime~" 
			} else {
